...
public class PartorderList {
	...
	public static ArrayList<PartorderList> parseData(String response)
			throws Exception {

		ArrayList<PartorderList> custlist = new ArrayList<>();

		PartorderList detail = new PartorderList();
 
		JSONArray detailarray = new JSONArray();
		detailarray = new JSONArray(response);

		for (int i = 0; i < detailarray.length(); i++) {
			detail = new PartorderList();
			JSONObject userDetails = detailarray.getJSONObject(i);
			detail.setBranch(userDetails.getString(TAG_BRANCH));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_BRANCH));
			detail.setCustname(userDetails.getString(TAG_CUST_NAME));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_CUST_NAME));
			detail.setContact(userDetails.getString(TAG_CONTACT));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_CONTACT));
			detail.setCustno(userDetails.getString(TAG_CUST_NO));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_CUST_NO));
			detail.setOrderdate(userDetails.getString(TAG_ORDER_DATE));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_ORDER_DATE));
			detail.setOrderno(userDetails.getInt(TAG_ORDER_NO));
			Log.d("Result for porder list", "" + userDetails.getInt(TAG_ORDER_NO));
			detail.setShipto(userDetails.getString(TAG_SHIP_TO));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_SHIP_TO));
			detail.setSigndocid(userDetails.getInt(TAG_SIGNADOCID));
			Log.d("Result for porder list", "" + userDetails.getInt(TAG_SIGNADOCID));
			detail.setBranchName(userDetails.getString(TAG_BRANCH_NAME));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_BRANCH_NAME));
			detail.setCustomerPo(userDetails.getString(TAG_CUSTOMER_PO));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_CUSTOMER_PO));
			detail.setType(userDetails.getString(TAG_TYPE));
			Log.d("Result for porder list", "" + userDetails.getString(TAG_TYPE));
...
//			"signdocid":0,"branchname":"RHSI - ONTARIO","pono":"HUGO ALONZO","ordertype":"S"}]
			...
			custlist.add(detail);
			Log.d("Result for porder list", "" + custlist);
		}
		return custlist;
	}
