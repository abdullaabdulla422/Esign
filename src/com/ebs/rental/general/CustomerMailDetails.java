...
import com.ebs.rental.objects.Desc;
import com.ebs.rental.objects.Parts;
...
public class CustomerMailDetails extends AppCompatActivity implements OnClickListener {
    ...
    RentalDocumentDetail rentalDocumentDetail;
    private String str;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dis = new Parts();
        desc = new Desc();
        if (SessionData.getInstance().getFiltereddesc() != null) {
            arraysize = SessionData.getInstance().getFiltereddesc().size();
        ...
        for (int i = 0; i < arraysize; i++) {
            ...
            dis.setProperty(0, filterdesc.get(i));
            dis.setProperty(1, filterequip.get(i));
            desc.add(dis);
        }
        ...
        if (SessionData.getInstance().getSignData() != null) {
            signature = Base64.encodeToString(SessionData.getInstance()
                    .getSignData(), Base64.DEFAULT);
        }

        Log.d("signature data", "" + SessionData.getInstance().getSignData());
...
//		list.setAdapter(adapter);
        ...
        adap = new Adaptor(CustomerMailDetails.this, customerNameMails);
...
//                } else {
//                    gpstrack = new GPSTracker(CustomerMailDetails.this);
...
//            }
    ...
    public void onBackPressed() {
        ...
        super.onBackPressed();
        Intent intent = new Intent(CustomerMailDetails.this,
    ...
    private class AsynSubmitSigPdf extends AsyncTask<Void, Void, Void> {
        protected void onPreExecute() {
            ...
            ProgressBar.showCommonProgressDialog(CustomerMailDetails.this);
        }
            ...
            try {
                ...
                rentaorderdocpdf = WebServiceConsumer.getInstance()
                        .rentalOrderSignedDocumentPdf(
                                ...
                                SessionData.getInstance().getEquipID(),
                                Location, "data", "0");
            ...
            if (rentaorderdocpdf != null) {
                if (SessionData.getInstance().getRentalOrderSignedDocumentPdf() == 0) {
                    ...
                    if (rentaorderdocpdf.getMessage().contains("Session")) {
                        session="AsynSubmitSigPdf";
                        new AsyncLoginTask().execute();
                    } else {
                        dialog = new Dialog(CustomerMailDetails.this);
...
//								Intent inspection = new Intent(CustomerMailDetails.this,
//										MainActivity.class);
...
//				requestWindowFeature(Window.FEATURE_NO_TITLE);
                    ...
                    if (session == "AsyncUpdateContactEmails") {
                        new AsyncUpdateContactEmails().execute(customerNameMailsNew);
                    } else if (session == "AsynSubmitSigPdf") {
                        new AsynSubmitSigPdf().execute();
                    }
                ...
                ProgressBar.dismiss();
                AlertDialogBox.showAlertDialog(CustomerMailDetails.this,
                        "Data is not found");
            }
    ...
    private class AsynDocumentdet extends AsyncTask<Void, Void, Void> {
        protected void onPreExecute() {
        ...
        protected Void doInBackground(Void... params) {
            // TODO Auto-generated method stub
            // rentalDocumentDetail =
            // WebServiceConsumer.getInstance().getRentalDocumentDetail(user.getUserDescription(),
            // rentaorderdocpdf.getResult(), description, mfg,
            // model, SessionData.getInstance().getEquipID(), serialnum, rate);
        ...
        if (customerNameMails.size() > 0) {
            if (customerNameMails.get(customerNameMails.size() - 1).getEmail().length() != 0) {
                ...
                if (emailflag) {
                    ...
                    Toast.makeText(CustomerMailDetails.this, "Enter the Vaild Mail", Toast.LENGTH_LONG).show();
                }


            } else {
                Toast.makeText(CustomerMailDetails.this, "Enter the Mail", Toast.LENGTH_LONG).show();
            }
        } else {
            CustomerNameMail customerNameMail = new CustomerNameMail();
            customerNameMail.setIsAlreadyExit(0);
